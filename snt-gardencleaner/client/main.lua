                ...
                if distance < Config.Job.CarControl.DrawDistance-2 then
                    ...
                    if IsControlJustReleased(0,46) then
                        ...
                        if NumberToDo == 0 then
                            ...
                            TriggerServerEvent("snt-gardencleaner:givemoney", JobsToDo, SetRandomLocation)
                        end
                        CarParked = false
                        HasJobStarted = false
...
RegisterNetEvent('snt-gardencleaner:startjob')
AddEventHandler('snt-gardencleaner:startjob', function()
...
RegisterNetEvent('snt-gardencleaner:charvest')
AddEventHandler('snt-gardencleaner:charvest', function(data)
...
RegisterNetEvent('snt-gardencleaner:LeafBlower')
AddEventHandler('snt-gardencleaner:LeafBlower', function(data)
    ClearPedTasks(GetPlayerPed(-1))
...
RegisterNetEvent('snt-gardencleaner:collecttrash')
AddEventHandler('snt-gardencleaner:collecttrash', function(data)
    ClearPedTasks(GetPlayerPed(-1))
...
RegisterNetEvent('snt-gardencleaner:bin')
AddEventHandler('snt-gardencleaner:bin', function(data)
    haveBinBag = false
    POINTS_DONE_IN_JOB = POINTS_DONE_IN_JOB+1
...
RegisterNetEvent("snt-gardencleaner:putonclothes")
AddEventHandler("snt-gardencleaner:putonclothes", function()
    if ClothesOn then
        ChangeClothes()
    else
        ChangeClothes('work')
...
function SpawnStartingPed()
    ...
    if not Config.job then
        exports[Config.TargetName]:AddTargetEntity(spawnedPed, {
            options = {
                {
                    event = "snt-gardencleaner:startjob",
                    icon = Config.StartJobEmote,
                    label = Config.Languages[Config.Lang]["TARGET_STARTJOB"],
                },
                {
                    event = "snt-gardencleaner:putonclothes",
...
function SpawnLeafs()
    ...
    exports[Config.TargetName]:AddTargetEntity(p, {
        options = {
            {
                event = "snt-gardencleaner:LeafBlower",
                icon = Config.StartJobEmote,
...
function SpawnCharvest()
        ...
        exports[Config.TargetName]:AddTargetEntity(p, {
            options = {
                {
                    event = "snt-gardencleaner:charvest",
                    icon = Config.StartJobEmote,
...
function SpawnBinBags()
    ...
    exports[Config.TargetName]:AddTargetEntity(p, {
        options = {
            {
                event = "snt-gardencleaner:collecttrash",
                icon = Config.StartJobEmote,
